...
public final class Lab2 {
	...
	// ============================================================================================
	// IMPORTANT: the tester file passes some tests out-of-the-box ONLY because
	// arbitrary
	// ========== return values have been added to the methods to ensure Lab2.java
	// compiles
	//
	...
	// - if for some reason you cannot complete a method (to satisfy all of its
	// tests),
	 ...
	 * integer. Note that the sum might overflow if n is too large; this method does
	 * not check if the sum overflows (i.e., it's the client's problem)
	 * 
	 * 
	...
	public static int sum(int n) {
		...
		return n; // you need to replace this return statement
	}
	...
	public static boolean isPrime(int x) {
		 ...
		 * 
		 * instead, complete the private helper method isPrime(int, int) that is invoked
		 * from this method (see lab2 definition for more info)
		 * 
		 */
	...
	/**
	 * A privately defined helper method for isPrime(int), to recursively determine
	...
	/**
	 * Returns a string of length n formed by alternating the characters first and
	 * second. E.g.
	 * 
	 * <pre>
	...
	public static String alternate(char first, char second, int n) {
		...
		if (n == 0) {
			...
			n -= 1;
			if (n != 0) {
				alternating += second + alternate(first, second, n - 1);
			}
		}
		return alternating; // you need to replace this return statement
	}
	...
	public static String getParenthesis(String str) {
		int front = 0;
		int back = 0;  
		if (str.charAt(front) == '(' && str.charAt(str.length() - 1 - back) == ')') {
			return str;
		} else if (str.charAt(front) == '(') {
			back++;
			String newStr = str.substring(front, str.length() - back);
			return getParenthesis(newStr);
		} else if (str.charAt(str.length() - 1 - back) == ')') {
			front++;
			String newStr = str.substring(front, str.length() - back);
			return getParenthesis(newStr);
		} else {
	 ...
	 * (as a string). In a hexadecimal string, there are 16 digits: 0-9, then
	 * A,B,C,D,E,F representing 10-15 respectively.
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * This method should decompose the decimal (integer) value recursively (no
	 * loops allowed), by dividing the decimal value 16, and checking the remainder
	...
	public static String decimalToHex(int value) {
		...
		if (value > 0) {
			...
			if (remainder > 9) {
				hexString = hexCodes[remainder - 10];
			} else {
				hexString = Integer.toString(remainder);
			}
			return decimalToHex(value / 16) + hexString;
		}
		return hexString; // you need to replace this return statement
	}
}
