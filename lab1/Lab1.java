...
public class Lab1 {
	...
	public static double windChill(double airTemp, double windSpeed) {
		...
		return (13.12) + (0.6215 * airTemp) + (0.3965 * airTemp * Math.pow(windSpeed, 0.16))
	...
	public static String alternatingCaps(String s) {
		...
		for (int i = 0; i < s.length(); i++) {
			if (i % 2 == 0) {
				newString += Character.toLowerCase(s.toCharArray()[i]);
			} else {
				newString += Character.toUpperCase(s.toCharArray()[i]);
			}
		}
		return newString;
	}
